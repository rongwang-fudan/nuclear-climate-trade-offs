...
function [ npv, tip, S,emico2_0, costadd] = Abatementcn( FFlux, L, dpower, dcoef, tempopt, LR, covidyear, iec_cn, calrsav, deffs, techdiff, switcher, econcn, abtcn, tempdiff, weitzman, dicetfp, tipping, emi_neg, optlrsav, elasmu, rour, scc, lim_t,nuccnlist,CONSTR_nuc,GDPcorrect_Pforacc,popcorrect_Pforacc )

global Egreen realtime S0 cou_iform theta2 alpha fudanccm_exo EFco2 burkecoef tempzone cndata cnenergy nuclear_tip nuclear_tech nuclear_acc randnumc invest_trans energy_growth mac_curves neg_year nuclear_D_GDP abty INES_sen  NPG_sen sen_open
...
log10_nuclear_0=zeros(cn_num,1);
if fudanccm_exo==3
    % DICE
    ...
    if nuclear_tip>0 && totalemico2>0 && t>=121 && realtime(t+1,1)>abtcn(cn,7)
        ...
        for m=1:size(nuccnlist,2)
            ...
            elseif nuclear_tech==3
                ...
                %                 cum_nuclear = sum(Nuc(121:t,cn+1)) + max(1,(sum(cnenergy(cn,1:49,6),2)+cnenergy(cn,49,6)*5) * 0.0928); % national tech advances (default)
            end

            % cumulative nuclear power PWh that will have a new accident
            if NPG_sen==0
                log10_nuclear = 0.512 * log10(cum_nuclear) - 1.816;
            elseif NPG_sen==1
                log10_nuclear = 0.512 * log10(cum_nuclear) - 1.816 - 0.8385*1;
            elseif NPG_sen==2
                if t<122
                    log10_nuclear = 0.512 * log10(cum_nuclear) - 1.816;
            ...
            Dnuclear_0=0;
            if INES_sen==0
                nucP=[0.318	0.407	0.247	0.0208	0.00456	0.00187 0	0.00159];
            else
                nucP=[0.31016731	0.427284427	0.238095238	0.018018018	0.002574003	0.001287001	0	0.002574003];
            end
                    ...
                    for n=1:8
                        ...
                        elseif nuclear_acc==5
                            if nulevel<5
                                ...
                                elseif nuclear_D_GDP==2
                                    Dnuclear_0=(Dam_area*cn_labor*(5.3/10000*Dam_R(nulevel-4)+0.1397*Dam_D(nulevel-4))+Dam_worker(nulevel-4)+Dam_clean(nulevel-4))* (1+rour*0.001)^(t-120)+Dam_area_G*S(61,13,cn+1)*Dam_disc;
                                end
            ...
            if nuclear_tech==5
                ...
                costadd(t+1,1,cn+1)=newnuclear2 * 2;
                costadd(t+1,2,cn+1)=S(t+1,38,cn+1) *(0.0046+0.0046*2+0.002);
            else
                ...
                costadd(t+1,1,cn+1)=newnuclear2 * 6.317;
                costadd(t+1,2,cn+1)=S(t+1,38,cn+1) *(0.0046+0.0046*2+0.002);
            end
