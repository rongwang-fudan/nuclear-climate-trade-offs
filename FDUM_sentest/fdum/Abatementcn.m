...
function [ npv, tip, S,emico2_0] = Abatementcn( FFlux, L, dpower, dcoef, tempopt, LR, covidyear, iec_cn, calrsav, deffs, techdiff, switcher, econcn, abtcn, tempdiff, weitzman, dicetfp, tipping, emi_neg, optlrsav, elasmu, rour, scc, lim_t,nuccnlist,CONSTR_nuc,GDPcorrect_Pforacc,popcorrect_Pforacc )

global Egreen realtime S0 cou_iform theta2 alpha fudanccm_exo EFco2 burkecoef tempzone cndata cnenergy nuclear_tip nuclear_tech nuclear_acc randnumc invest_trans energy_growth mac_curves neg_year nuclear_D_GDP abty sen_NPG sen_tip sen_INES sen_open nucrour Genrour
...
DinY=zeros(400,cn_num,4);
tlast=ones(cn_num,1);
tlast=tlast*121;
tipY=zeros(5,1);
tipY_record=zeros(5,1);
if fudanccm_exo==3
    % DICE
...
while realtime(t,1)<endyear
    ...
    cn_t0=cn_yabt-cn_alen/100*(cn_yabt-abty);
    ...
    for tip_i=1:5
        %         Without interaction
        tip_prob_2(tip_i,1)=1-exp(-tip_para(tip_i,1)/100*max(0,tempw-lim_t));% Pij the probability for tipping point i to change its status from "pre-trigger" to "post-trigger" in year t
        tip_prob_2(tip_i,3)=1-(1-tip_prob_2(tip_i,3))*(1-tip_prob_2(tip_i,1));%Theta:the probability of a tipping point
        %         Interaction
        interact=0;
        interact_2=0;
        for tip_j=1:5
            ...
            interact_2=interact_2+tip_fij(tip_j,tip_i)*tip_prob_2(tip_j,4);
            %             end
        end
        tip_prob_2(tip_i,2)=1-exp(-tip_para(tip_i,1)/100*max(0,tempw-lim_t)*(1+interact_2));% Pij the probability for tipping point i to change its status from "pre-trigger" to "post-trigger” in year t

        tip_prob_2(tip_i,4)=1-(1-tip_prob_2(tip_i,2))*(1-tip_prob_2(tip_i,4));%Theta:the probability of a tipping point


        tip(t,tip_i)=1-tip_prob_2(tip_i,4); % (theta)Probabiity of n(theta)o tipping
        tip_2(t,tip_i)=1-tip_prob_2(tip_i,3);% (theta)Probabiity of no tipping with out interaction
        tip_3(t,tip_i)=1-tip_prob_2(tip_i,2); % (Pij)Probabiity of no tipping
        tip_4(t,tip_i)=1-tip_prob_2(tip_i,1);% (Pij)Probabiity of no tipping with out interaction
        if t>120
            tip_sum_2(1)=tip_sum_2(1)+(t)*(1-tip_4(t,tip_i));%without interaction
            tip_sum_2(2)=tip_sum_2(2)+max(1-tip_4(t,tip_i),0.000000001);
            tip_sum_1(1)=tip_sum_1(1)+(t)*(1-tip_3(t,tip_i));
            tip_sum_1(2)=tip_sum_1(2)+max(1-tip_3(t,tip_i),0.000000001);
        end
        tip_prob(tip_i,1)=tip_sum_2(1)/tip_sum_2(2); % τ is a year before year t without interaction
        tip_prob(tip_i,2)=tip_sum_1(1)/tip_sum_1(2);
    end
    ...
    if tipping>0
        for tip_i=1:5
            if tipping==2
                ...
                tip_p=1-tip_2(t,tip_i);
            end
            if sen_open(3)==0
                tipY(tip_i)=tip_prob(tip_i,tipping);
            else
                if tip_p<sen_tip
                    tip_p=0;
                else
                    tip_p=1;
                    tipY_record(tip_i)=tipY_record(tip_i)+1;
                    if tipY_record(tip_i)==1
                        tipY(tip_i)=t;
                    end
                end
            end
            ...
            if tipping<=2
                damage_tip_i = tip_p*tip_para(tip_i,3)/100 * min(1,max( 0,(t-tipY(tip_i))/tip_para(tip_i,2) )) ;
            else
                damage_tip_i = tip_p*tip_para(tip_i,3)/100 * min(1,max( 0,(t-tipY(tip_i)/ 10 ) ));
            end
            damage_tip = damage_tip * (1-damage_tip_i);
            tip(t,tip_i+5) = 1-damage_tip_i;
        end
    end
    ...
    for cn=1:cn_num
        ...
        %Utility
        if (t+1)>=121 && (t+1)<=400
            S(t+1,36,cn+1) = (((1-rsav)* S(t+1,7,cn+1)/S(t+1,35,cn+1)*1000)^(1-elasmu)/(1-elasmu)) * S(t+1,35,cn+1) /1000  * (1-rour*0.001)^(t-abty+1904+1); % utility discounted to 2025
        end
    end
    ...
    [nuc_a,nuc_b]=find(cndata(:,1)==nuccnlist);

    totalemico2=sum(S(t+1,20,nuc_a+1),3); % total emissions for 33 country with nuclear power
    if nuclear_tip>0 && totalemico2>0 && t>=121 && realtime(t+1,1)>abtcn(cn,7)
        ...
        for m=1:size(nuccnlist,2)
            ...
            if t<121+CONSTR_nuc
                S(t+1,40,cn+1) =0;
            elseif t>120+CONSTR_nuc && t<131+CONSTR_nuc
                lim_F=min(0.85, S(t+1,20,cn+1) / max(0.000001, newnuclear * 8.76 * 0.8433));
                S(t+1,40,cn+1)=min(0.85/10*(t-(120+CONSTR_nuc)),lim_F);
            else
            ...
            if sen_open(1)==0
                log10_nuclear = 0.512 * log10(cum_nuclear) - 1.816;%0.8385 std of NPG
            elseif sen_open(1)==1
                log10_nuclear = 0.512 * log10(cum_nuclear) - 1.816 + sen_NPG*0.8385+0.364*Genrour;%0.8385 std of NPG +2.00*
            elseif sen_open(1)==2
                if t<122
                    log10_nuclear = 0.512 * log10(cum_nuclear) - 1.816+ sen_NPG*0.8385+0.364*Genrour;
                    log10_nuclear_0(cn+1)=log10_nuclear;
                else
                    log10_nuclear=log10_nuclear_0(cn+1);
            ...
            Dnuclear_0=0;
            if sen_open(2)==0
                nucP=[0.318	0.407	0.247	0.0208	0.00456	0.00187+(1-nucrour)*0.00159 0	0.00159*nucrour];
            else
                if sen_INES==1
                    nucP=[0.317744154	0.382393398	0.25997249	0.027510316	0.008253095	0.002751032+(1-nucrour)*0.001375516	0	0.001375516*nucrour];
                elseif sen_INES==2
                    nucP=[0.319736842	0.397368421	0.252631579	0.021052632	0.005263158	0.002631579+(1-nucrour)*0.001315789	0	0.001315789*nucrour];
                elseif sen_INES==3
                    nucP=[0.330718954	0.393464052	0.249673203	0.019607843	0.003921569	0.00130719+(1-nucrour)*0.00130719	0	0.00130719*nucrour];
                elseif sen_INES==4
                    nucP=[0.31016731	0.427284427	0.238095238	0.018018018	0.002574003	0.001287001+(1-nucrour)*0.002574003	0	0.002574003*nucrour];
                elseif sen_INES==5
                    nucP=[0.309623431	0.433751743	0.232914923	0.018131102	0.0027894	0.0013947+(1-nucrour)*0.0013947	0	0.0013947*nucrour];
                end
            end
            Dam_distance=[20 25 30];%for health
            Dam_D=[0 0 1];%for Death
            Dam_R=[0.08 1.5 33];% mSv radiation dose 0.08 3milesisland 33 Chernobyl
            Dam_distance_G=[2 5 25];%for GDP
            Dam_clean=[0.001 0.01 0.187];% 0.001 three miles island 0.187 fukushima
            Dam_worker=[0 0.01 0.0325];% 0.0325(mean) in fukushima 0.0478(high)
            if nuclear_tech~=4 && id_nuclear(cn-1,2)>cri_nuclear
                S(t+1,41,cn+1) = max(1,round( id_nuclear(cn-1,2)/cri_nuclear)); % number of accident
                ...
                for nui=1:S(t+1,41,cn+1)
                    for n=1:8
                        ...
                        elseif nuclear_acc==4
                            Dnuclear=Dnuclear+10^(nulevel*0.834-6.523);   % trillion $
                        elseif nuclear_acc==5
                            if nulevel<5
                                Dnuclear_0=10^(nulevel*0.239-1.690-3)*nucP(n);
                            else
                                % trillion $
                                Dam_area=Dam_distance(nulevel-4)^2*pi/cou_iform(cndata(cn,1),8);
                                Dam_area_G=Dam_distance_G(nulevel-4)^2*pi/cou_iform(cndata(cn,1),8);
                                if nuclear_D_GDP==1
                                    t_q=50;
                                    Dam_timelist=[1:-1/t_q:0]';
                                    Dam_timelist=Dam_timelist.*(Dam_area_G*S(t,13,cn+1)*GDPcorrect_Pforacc(nulevel-4+1,m));
                                    DinY(t+1:min(400,t+1+t_q),cn+1,nulevel-4)=DinY(t+1:min(400,t+1+t_q),cn+1,nulevel-4)+Dam_timelist(1:min(t_q+1,400+t_q-(t+t_q)));
                                    Dnuclear_0=(Dam_area*cn_labor*(5.3/10000*Dam_R(nulevel-4)+0.1397*Dam_D(nulevel-4))*popcorrect_Pforacc(nulevel-4+1,m)+Dam_worker(nulevel-4)+Dam_clean(nulevel-4));
                                    if nui==S(t+1,41,cn+1)
                                        Dnuclear_0=Dnuclear_0+DinY(t+1,cn+1,nulevel-4);
                                    end
                                end
                                Dnuclear_0=Dnuclear_0*nucP(n);

                            end
                        end
                        Dnuclear=Dnuclear+Dnuclear_0;
                ...
                if t>120
                    reducenum=1;
                    Pines7=nucP(8)*sum(S(tlast(cn+1):t+1,41,cn+1),1);
                    if Pines7>1
                        tlast(cn+1)=t+1;
                        reducenum=0.9;
                    end
                    if t+1<tlast(cn+1)+10 && tlast(cn+1)>121
                        reducenum=0.9*(1+(t+1-tlast(cn+1))*0.01);
                    end
                    S(t+1,39,cn+1) =S(t+1,39,cn+1) *reducenum;
                end
            ...
            if nuclear_tech==5
                S(t+1,43,cn+1) = newnuclear2 * 2 + S(t+1,38,cn+1) * (0.0046+0.0046*2+0.002); % construction costs of 2000 $/kW and 0.0055 $/kWh for uranium cost (51% in operational costs)
            else
                S(t+1,43,cn+1) = newnuclear2 * 6.317 + S(t+1,38,cn+1) * (0.0046+0.0046*2+0.002); % default: construction costs of 4000 $/kW and 0.0055 $/kWh for uranium cost (51% in operational costs)
            end
    ...
    %Negative emissions
    if realtime(t,1)<2025
        emi_neg0 = 0; % deploying negative emissions since 2025
    else
        emi_neg0 = emi_neg-emi_neg*exp(-(realtime(t,1)-2025)^2/(-(neg_year-2025)^2/log(0.1))); % deploying emi_neg after 2025
    end
