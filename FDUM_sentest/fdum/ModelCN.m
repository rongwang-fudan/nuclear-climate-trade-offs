...
global elas carbonbudget20 Eland EFco2 Fex cndata deltarf fudanccm_exo cou_iform burkecoef tempzone nuclear_tip nuclear_tech nuclear_acc randnumc invest_trans energy_growth mac_curves neg_year nuclear_D_GDP abty sen_NPG sen_tip sen_INES sen_open nucrour Genrour
...
sen_open=[0 0 0]; % 0 central case 1 sensitivity
abty=2020;
nucrour=1;
NPG_z=[1.96	1.64	1.44	1.28	1.15	1.04	0.93	0.84	0.76	0.67	0.6	0.52	0.45	0.39	0.32	0.25	0.19	0.13	0.06	0	-0.06	-0.13	-0.19	-0.25	-0.32	-0.39	-0.45	-0.52	-0.6	-0.67	-0.76	-0.84	-0.93	-1.04	-1.15	-1.28	-1.44	-1.64	-1.96];
sen_NPG=NPG_z(5);
tip_z=[0.025:0.025:0.975];
sen_tip=0;
sen_INES=1;
Genrour=0;
unc=ones(1,19);
...
elseif mcmode==9 % scc-nuclear-stochastic
    ...
    nuclear_tech=1; % 0 no tech advances; 1 national tech advances; 2 global cooperation; 3 OECD/REF/ALM/ASIA cooperation; 4 no accident; 5 low nuclear costs ($2/We); 6 lifetime 20 yrs
    nuclear_acc=5; % 1 damage as a % of GDP; 2 damage as a % of GDP forced (calibrated to level 7); 3 damage as absolute $; 4 damage as absolute $ (calibrated to level 7)
    nzlenlist=[7 14 23 27 33];
    if mc==1
        nzlen=20; % delayed mitigation
        nuclear_tech=1; 
    elseif mc>1 && mc<size(codelist,1)+2
        nzlen=nzlenlist(1); % 1.5Ctest
        nuclear_tech=1;
        esc2=3;nonCO2=0.5;rour=20;
        neg=15;
        code=mc-1;
        sen_NPG=NPG_z(codelist(code,1));
        sen_tip=tip_z(codelist(code,3));
        sen_INES=codelist(code,2);
        sen_open=codelist(code,4:6);
    elseif mc>size(codelist,1)+1 && mc<size(codelist,1)*2+2
        nzlen=nzlenlist(2); % 2Ctest
        nuclear_tech=1; 
        esc2=3;nonCO2=0.5;rour=20;
        neg=5;
        code=mc-1-size(codelist,1);
        sen_NPG=NPG_z(codelist(code,1));
        sen_tip=tip_z(codelist(code,3));
        sen_INES=codelist(code,2);
        sen_open=codelist(code,4:6);
    elseif mc>size(codelist,1)*2+1 && mc<size(codelist,1)*3+2
        nzlen=nzlenlist(3); % test3
        nuclear_tech=1; % 
        esc2=3;nonCO2=0.6;rour=20;neg=0;
        code=mc-1-size(codelist,1)*2;
        sen_NPG=NPG_z(codelist(code,1));
        sen_tip=tip_z(codelist(code,3));
        sen_INES=codelist(code,2);
        sen_open=codelist(code,4:6);
    elseif mc>size(codelist,1)*3+1 && mc<size(codelist,1)*4+2
        nzlen=nzlenlist(5); % test5
        nuclear_tech=1; 
         esc2=3;nonCO2=0.7;rour=20;neg=0;
         code=mc-1-size(codelist,1)*3;
        sen_NPG=NPG_z(codelist(code,1));
        sen_tip=tip_z(codelist(code,3));
        sen_INES=codelist(code,2);
        sen_open=codelist(code,4:6);
    ...
    elseif mc>size(codelist,1)*4+1 && mc<size(codelist,1)*5+2
        nzlen=nzlenlist(1); % 1.5Ctest
        nuclear_tech=1;
        esc2=3;nonCO2=0.5;rour=20;
        neg=15;
        code=mc-1-size(codelist,1)*4;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
        nucrour=codelist2(code,7);
    elseif mc>size(codelist,1)*5+1 && mc<size(codelist,1)*6+2
        nzlen=nzlenlist(2); % 2Ctest
        nuclear_tech=1; 
        esc2=3;nonCO2=0.5;rour=20;
        neg=5;
        code=mc-1-size(codelist,1)*5;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
        nucrour=codelist2(code,7);
    elseif mc>size(codelist,1)*6+1 && mc<size(codelist,1)*7+2
        nzlen=nzlenlist(3); % test3
        nuclear_tech=1; % 
        esc2=3;nonCO2=0.6;rour=20;neg=0;
        code=mc-1-size(codelist,1)*6;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
        nucrour=codelist2(code,7);
    elseif mc>size(codelist,1)*7+1 && mc<size(codelist,1)*8+2
        nzlen=nzlenlist(5); % test5
        nuclear_tech=1; 
         esc2=3;nonCO2=0.7;rour=20;neg=0;
         code=mc-1-size(codelist,1)*7;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
    ...
    elseif mc>size(codelist,1)*8+1 && mc<size(codelist,1)*9+2
        nzlen=nzlenlist(1); % 1.5Ctest
        esc2=3;nonCO2=0.5;rour=20;
        neg=15;
        code=mc-1-size(codelist,1)*8;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
        nucrour=codelist2(code,7);
        nuclear_tech=5;
    elseif mc>size(codelist,1)*9+1 && mc<size(codelist,1)*10+2
        nzlen=nzlenlist(2); % 2Ctest
        esc2=3;nonCO2=0.5;rour=20;
        neg=5;
        code=mc-1-size(codelist,1)*9;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
        nucrour=codelist2(code,7);
        nuclear_tech=5;
    elseif mc>size(codelist,1)*10+1 && mc<size(codelist,1)*11+2
        nzlen=nzlenlist(3); % test3
        esc2=3;nonCO2=0.6;rour=20;neg=0;
        code=mc-1-size(codelist,1)*10;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
        nucrour=codelist2(code,7);
        nuclear_tech=5;
    elseif mc>size(codelist,1)*11+1 && mc<size(codelist,1)*12+2
        nzlen=nzlenlist(5); % test5
         esc2=3;nonCO2=0.7;rour=20;neg=0;
         code=mc-1-size(codelist,1)*11;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
     ...
     elseif mc>size(codelist,1)*12+1 && mc<size(codelist,1)*13+2
        nzlen=nzlenlist(1); % 1.5Ctest
        nuclear_tech=1;
        CONSTR_nuc=20;
        esc2=3;nonCO2=0.5;rour=20;
        neg=15;
        code=mc-1-size(codelist,1)*12;
        sen_NPG=NPG_z(codelist(code,1));
        sen_tip=tip_z(codelist(code,3));
        sen_INES=codelist(code,2);
        sen_open=codelist(code,4:6);
    elseif mc>size(codelist,1)*13+1 && mc<size(codelist,1)*14+2
        nzlen=nzlenlist(2); % 2Ctest
        nuclear_tech=1; 
        CONSTR_nuc=20;
        esc2=3;nonCO2=0.5;rour=20;
        neg=5;
        code=mc-1-size(codelist,1)*13;
        sen_NPG=NPG_z(codelist(code,1));
        sen_tip=tip_z(codelist(code,3));
        sen_INES=codelist(code,2);
        sen_open=codelist(code,4:6);
    elseif mc>size(codelist,1)*14+1 && mc<size(codelist,1)*15+2
        nzlen=nzlenlist(3); % test3
        nuclear_tech=1; % 
        CONSTR_nuc=20;
        esc2=3;nonCO2=0.6;rour=20;neg=0;
        code=mc-1-size(codelist,1)*14;
        sen_NPG=NPG_z(codelist(code,1));
        sen_tip=tip_z(codelist(code,3));
        sen_INES=codelist(code,2);
        sen_open=codelist(code,4:6);

    elseif mc>size(codelist,1)*15+1 && mc<size(codelist,1)*16+2
        nzlen=nzlenlist(5); % test5
        nuclear_tech=1; 
        CONSTR_nuc=20;
         esc2=3;nonCO2=0.7;rour=20;neg=0;
         code=mc-1-size(codelist,1)*15;
        sen_NPG=NPG_z(codelist(code,1));
        sen_tip=tip_z(codelist(code,3));
        sen_INES=codelist(code,2);
        sen_open=codelist(code,4:6);

         %test for Generation and cost

    elseif mc>size(codelist,1)*16+1 && mc<size(codelist,1)*17+2
        nzlen=nzlenlist(1); % 1.5Ctest
        esc2=3;nonCO2=0.5;rour=20;
        neg=15;
        code=mc-1-size(codelist,1)*16;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
        ...
        nuclear_tech=5;
        Genrour=1;
    elseif mc>size(codelist,1)*17+1 && mc<size(codelist,1)*18+2
        nzlen=nzlenlist(2); % 2Ctest
        esc2=3;nonCO2=0.5;rour=20;
        neg=5;
        code=mc-1-size(codelist,1)*17;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
        ...
        nuclear_tech=5;
        Genrour=1;
    elseif mc>size(codelist,1)*18+1 && mc<size(codelist,1)*19+2
        nzlen=nzlenlist(3); % test3
        esc2=3;nonCO2=0.6;rour=20;neg=0;
        code=mc-1-size(codelist,1)*18;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
    ...
    elseif mc>size(codelist,1)*19+1 && mc<size(codelist,1)*20+2
        nzlen=nzlenlist(5); % test5
         esc2=3;nonCO2=0.7;rour=20;neg=0;
         code=mc-1-size(codelist,1)*19;
        sen_NPG=NPG_z(codelist2(code,1));
        sen_tip=tip_z(codelist2(code,3));
        sen_INES=codelist2(code,2);
        sen_open=codelist2(code,4:6);
...
for s=16:-1:1
    ...
    if mcmode==8 || mcmode==9
        ncp=[0:0.1:1.5].*test_num_N; ncp(12:16)=[2 3 4 5 6].*test_num_N; 
        abtcn(1:cn_num,1)=abty;
        abtcn(1:cn_num,2)=1;
    ...
    [npv, tip, S, emico2_0] = Abatementcn( FFlux, L, dpo, dcoef, tempopt, LR, covidyear, iec_cn, calrsav, deffs, techdiff, switcher, econcn, abtcn, tempdiff, weitzman, dicetfp, tipping, neg, optlrsav, elasmu, rour, scc,  lim_t, nuccnlist,CONSTR_nuc,GDPcorrect_Pforacc,popcorrect_Pforacc);
